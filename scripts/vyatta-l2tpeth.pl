...
if ( $action eq 'SET' ) {
	...
	$old_cookie =
	  [ "cookie", $old_local_cookie, "peer_cookie", $old_peer_cookie ]
...
sub delete_tun_session {
    ...
    system( "ip", "l2tp", "del", "session", "tunnel_id", $old_session,
...
sub add_new_session {
    ...
    system(
        "ip",        "l2tp",   "add",            "tunnel",
        "tunnel_id", $tunnel,  "peer_tunnel_id", $peer_tunnel,
        "encap",     $encap,   "local",          $local_ip,
        "remote",    $peer_ip, @$port
      ) == 0
      or exit 1;
    ...
    system(
        "ip",              "l2tp",        "add",        "session",
        "tunnel_id",       $tunnel,       "session_id", $session,
        "peer_session_id", $peer_session, "name",       $ifname,
        "l2spec_type",     "none",        @$cookie
      ) == 0
      or exit 1;
}
